<template>
	<div>
					...
					<v-col cols="12" md="5" sm="5">
						<v-hover v-slot="{ hover }">
							<v-avatar class="profile" size="108px">
								...
								<v-img :src="user.avatar" :lazy-src="require('@/assets/upload.png')" rounded />
							</v-avatar>
						</v-hover>
					</v-col>
		...
		<dialog-edit v-if="user.id && dialogs.edit" v-model="dialogs.edit" @close="dialogs.edit = false" :user="user" />
		<dialog-image v-if="dialogs.image" v-model="dialogs.image" @close="dialogs.image = false" :user="user" />
	</div>
</template>
