<template>
	<div>
			...
			<v-col cols="12" md="3">
				<v-row class="pa-3 pos-rel mb-2 grey lighten-4">
					...
					<v-col cols="6">
						...
						<div>
							<a :href="'http://' + supplier.website" style="text-decoration: none; color: #03616f" target="_blank">{{
								supplier.website
							}}</a>
              ...
              <a :href="supplier.instagram" style="text-decoration:none;"><v-icon>mdi-instagram</v-icon></a> -->
				...
				<v-row class="pa-3 pos-rel mb-2 grey lighten-4">
					...
					<v-col cols="6">
						<p class="spc-titles">סטטוס ספק</p>
						{{ supplier.status }}
					</v-col>
					<v-col cols="6">
						<p class="spc-titles">שעות פעילות</p>
			...
			<v-col cols="12" md="9" class="pr-10">
				...
				>
					...
					<template v-slot:expanded-item="{ headers, item }">
						<td class="orderWorkInfo" :rowspan="headers.length">
							{{ item.orderWork }}
						</td>
					</template>
		...
		<dialog-edit
			...
			:supplier="supplier"
		/>
		<dialog-image v-if="dialogs.image" v-model="dialogs.image" @close="dialogs.image = false" :supplier="supplier" />
		<dialog-create
			v-if="dialogs.create"
...
export default {
			...
			get() {
				return this.$store.state.Order.list
					...
					.filter(order => {
						...
						return (
							order.orderSupplierRef.id === this.supplier.id &&
	...
	components: {
		'dialog-edit': require('@/components/Suppliers/Dialogs/DialogEdit').default,
		'dialog-create': require('@/components/Orders/Dialogs/DialogCreate').default,
		'nav-appbar': require('@/components/Global/AppBar').default,
		'dialog-image': require('@/components/Suppliers/Dialogs/DialogImage').default,
		'dialog-order': require('@/components/Orders/Dialogs/DialogImage').default
	}
}
</script>
